...
int main(int argc, char* argv[])
{
    ...
    int send_chn_idx = 0;       // 发送通道

    // // CAN
    // ZCAN_20_MSG can_msg[send_num];  // 数组不规范写法，仅供参考
    // for (int i = 0; i < send_num; i++)
    //     construct_can_frame(&can_msg[i], i, send_chn_idx, 0);
    // send_ret = VCI_Transmit(DevType, DevIdx, send_chn_idx, &can_msg[0], send_num);
    // printf("send can frams %d\n", send_ret);

    // // CANFD
    // ZCAN_FD_MSG canfd_msg[send_num];        
    // for (int i = 0; i < send_num; i++)
    //     construct_canfd_frame(&canfd_msg[i], i, send_chn_idx, 0);
    // send_ret = VCI_TransmitFD(DevType, DevIdx, send_chn_idx, &canfd_msg[0], send_num);
    // printf("send canfd frams %d\n", send_ret);

    // 队列发送
    // queue_send(DevType, DevIdx, send_chn_idx);

    // 定时发送
    auto_send(DevType, DevIdx, send_chn_idx);
